...
import {StratigicObjService} from './stratigic-obj.service';
import {UsersService} from '../../Services/Users/users.service';


declare var $: any;
declare var stiffChart: any;
...
@Component({
  selector: 'app-strategic-objectives-charts',
  templateUrl: './strategic-objectives-charts.component.html',
  styleUrls: ['./strategic-objectives-charts.component.css'],
  providers: [StratigicObjService]
})
export class StrategicObjectivesChartsComponent implements OnInit {
  ...
  PerformSettings() {
    this.stratigicService.LoadYears().subscribe(res => {
      ...
      if (res.Data.length > 0) {
        this.ddlYearSearch = res.Data[res.Data.length - 1].id;
        this.LoadChart();
      }
    });
  }
  ...
  onProjectsListClick(id) {
    document.getElementById("addAssessmentModal").style.display = 'block';
    document.getElementById("addAssessmentModal").className = "modal fade in";
    this.AssessmentModal.setObjDefaultAddOnFly(id.split('_')[0]);
  }
  ...
  onAddObjecitveKPIsResult(id) {
    ...
    this.ObjectiveKpiModal.setObjDefaultAddOnFly(id.split('_')[0]);
  }
