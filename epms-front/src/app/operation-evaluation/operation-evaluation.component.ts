...
@Component({
  selector: 'app-operation-evaluation',
  templateUrl: './operation-evaluation.component.html',
  styleUrls: ['./operation-evaluation.component.css']
})
export class OperationEvaluationComponent implements OnInit {
  ...
  getEvalutions(name) {
    this.projectEvaluationService.GetProjectEvaluation(name, 2).subscribe(res => {
      if (res.IsError) {
...
// check weight value
    ...
    this.projectEvaluationService.SaveProjectEvaluation(this.userContextService.Username, this.txtProjectEvaluationName, this.projectEvaluationWeight, 2).subscribe(
      res => {
